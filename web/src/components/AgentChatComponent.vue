...
const preserveFailedHumanMessage = (
  threadId,
  threadState,
  requestId,
  { text = '', imageContent = null, attachments = [] } = {}
) => {
  if (!threadId || !threadState || !requestId) return
  const optimisticMessage =
    threadState.onGoingConv.msgChunks[requestId]?.find(
...
const handleSendMessage = async ({
  ...
  try {
    ...
    const isQueued = status === 'queued' || (!runId && status !== 'rejected')
    if (!isQueued && !runId) {
      throw new Error('创建 run 失败：缺少 run_id')
    }

    requestAccepted = true
    // 服务端已接纳请求（立即运行或进入队列）后，旧的本地失败副本和告警都应收敛。
    // 若本次运行随后失败，Run SSE 会写入新的失败事实与单个告警。
    ...
    // 请求已经被服务端接纳时，后续只是队列/Run SSE 订阅失败，不能伪造成发送失败。
    if (requestAccepted) {
      handleChatError(error, 'stream')
      return
    }

    const interruptedConflict = isRunInterruptedConflict(error)
    if (!interruptedConflict) {
    ...
    let alertId = null
    if (!interruptedConflict) {
      alertId = pushAlertFromError(threadId, error, {
        retry: {
          text,
          imageContent,
          attachments: pendingAttachments,
      ...
      if (alertId) {
        try {
          ...
          // 网络异常时仍保留当前页面的临时失败消息和告警。
        }
      }
    }
    if (!alertId) handleChatError(error, 'send')
  }
}
