...
def _register_search_tool() -> None:
    """注册统一搜索链，同时兼容上游 WEB_SEARCH_PROVIDER 和 preview 多 backend 配置。"""

    from yuxi.agents.toolkits.buildin.search import create_search_tool, resolve_search_backend, search_tool_metadata

    backend = resolve_search_backend()
    search_instance = create_search_tool()
    if search_instance is None:
        logger.warning("Search tool unavailable: backend=%s", backend)
        return
    metadata = search_tool_metadata(backend)
    _extra_registry[search_instance.name] = ToolExtraMetadata(
        category="buildin",
        tags=["搜索"],
...
# 保留上游内部测试和扩展代码使用的函数名；两者注册的是同一个 web_search 工具。
_register_web_search_tool = _register_search_tool
