...
async def can_skill_depend_on(db: AsyncSession, parent: Skill, dependency: Skill) -> bool:
    ...
    dep_raw = dependency.share_config or {}
    parent_raw = parent.share_config or {}
    if dep_raw.get("version") != 2 or parent_raw.get("version") != 2:
        return await _legacy_skill_scope_contains(db, parent_raw, dep_raw)

    dep_config = normalize_permission_config(dep_raw)
    parent_config = normalize_permission_config(parent_raw)
...
async def _legacy_skill_scope_contains(db: AsyncSession, parent_config: dict, dependency_config: dict) -> bool:
    """兼容 preview 已有组织树共享配置，直至历史数据全部迁移为 v2。"""

    dependency_level = dependency_config.get("access_level")
    parent_level = parent_config.get("access_level")
    if dependency_level == "global":
        return True
    if parent_level == "global":
        return False
    if parent_level == "user" and dependency_level == "user":
        parent_uids = {str(value) for value in parent_config.get("user_uids") or []}
        dependency_uids = {str(value) for value in dependency_config.get("user_uids") or []}
        return parent_uids.issubset(dependency_uids)
    if parent_level != "department" or dependency_level != "department":
        return False
    ...
    parent_uids = {str(value) for value in parent_config.get("user_uids") or []}
    dependency_uids = {str(value) for value in dependency_config.get("user_uids") or []}
    return allowed_departments(parent_config).issubset(allowed_departments(dependency_config)) and parent_uids.issubset(
        dependency_uids
    )
