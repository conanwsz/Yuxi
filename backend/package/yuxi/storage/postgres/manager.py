        ...
        stmts = [
            ...
            *role_seed_statements,
            ...
            (
                ...
                '\'{"access_level": "global", "department_ids": [], "user_uids": []}\'::jsonb '
                "ELSE share_config END, 'manage_scope', CASE "
                "WHEN manage_config = '{}'::jsonb THEN NULL "
                "WHEN manage_config->>'version' = '2' THEN "
                "COALESCE(manage_config->'manage_scope', manage_config->'read_scope') "
                "ELSE manage_config END) "
                "WHERE share_config IS NOT NULL AND share_config->>'version' IS DISTINCT FROM '2'"
            ),
            (
                "UPDATE agents SET share_config = jsonb_set(share_config, '{manage_scope}', "
                "CASE WHEN manage_config->>'version' = '2' THEN "
                "COALESCE(manage_config->'manage_scope', manage_config->'read_scope') "
                "ELSE manage_config END, true) "
                "WHERE share_config->>'version' = '2' "
                "AND share_config->'manage_scope' = 'null'::jsonb "
                "AND manage_config IS NOT NULL AND manage_config <> '{}'::jsonb"
            ),
