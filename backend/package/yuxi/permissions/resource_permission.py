...
def normalize_permission_config(
    ...
    *,
    ...
    # preview 历史数据只保存一个读取范围。读取时投影为 v2，管理权仍由
    # created_by 决定；新写入继续由 strict 模式强制使用 v2。
    normalized = {
        "version": 2,
        "read_scope": _normalize_scope(config or DEFAULT_SCOPE),
        "manage_scope": None,
    }
    if allowed_access_levels is not None and normalized["read_scope"]["access_level"] not in allowed_access_levels:
        raise ValueError(unauthorized_access_level_message)
    return normalized
...
def scope_matches(user: Any, scope: dict | None) -> bool:
    ...
    if not scope:
        return False
    return share_config_allows_user(user, scope)
