...
async def test_oidc_cnnp_exposes_entity_and_dept_code_env(test_client, standard_user):
    """OIDC 用户部门带 CNNP 编码时，entity_code / dept_code 与 emp_no 一起以 readonly 暴露。"""
    ...
    assert set(body["readonly_keys"]) == {"emp_no", "entity_code", "dept_code"}
...
async def test_oidc_user_without_organization_codes_gets_only_uid(test_client, standard_user):
    """部门没有 CNNP 编码时，OIDC 用户仍只暴露 emp_no，避免暴露空字段。"""
    ...
    assert response.status_code == 200, response.text
    body = response.json()
    assert body["env"] == {"emp_no": user["uid"]}
    assert body["readonly_keys"] == ["emp_no"]
