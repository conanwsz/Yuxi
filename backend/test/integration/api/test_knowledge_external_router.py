...
async def _create_restricted_database(test_client, admin_headers, embedding_model_spec):
    """Create a KB shared only with its creator (admin), so non-owners get 404."""
    current_user_response = await test_client.get("/api/auth/me", headers=admin_headers)
    assert current_user_response.status_code == 200, current_user_response.text
    owner_uid = current_user_response.json()["uid"]
...
async def test_external_access_is_restricted_to_owner(
    test_client,
    admin_headers,
    standard_user,
    embedding_model_spec,
):
    database = await _create_restricted_database(test_client, admin_headers, embedding_model_spec)
