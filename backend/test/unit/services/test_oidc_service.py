...
async def test_standard_provider_preserves_token_http_contract(monkeypatch, httpx_mock):
    monkeypatch.setattr(oidc_service.oidc_config, "client_id", "client-id")
    monkeypatch.setattr(oidc_service.oidc_config, "client_secret", "client-secret")
    monkeypatch.setattr(
    ...
    async def fake_metadata(cls):
        return SimpleNamespace(token_endpoint="https://issuer.example/token")

    monkeypatch.setattr(oidc_service.OIDCUtils, "get_metadata", classmethod(fake_metadata))
    httpx_mock.add_response(
        method="POST",
        url="https://issuer.example/token",
        json={"access_token": "access-token", "id_token": "id-token", "token_type": "Bearer"},
    )

    token_set = await oidc_service.OIDCUtils.exchange_code_for_token("authorization-code", "pkce-verifier")
    ...
    assert parse_qs(request.content.decode()) == {
        ...
        "redirect_uri": ["https://yuxi.example/api/auth/oidc/callback"],
        "client_id": ["client-id"],
...
async def test_standard_provider_preserves_userinfo_http_contract(monkeypatch, httpx_mock):
    async def fake_metadata(cls):
        return SimpleNamespace(userinfo_endpoint="https://issuer.example/userinfo")

    monkeypatch.setattr(oidc_service.OIDCUtils, "get_metadata", classmethod(fake_metadata))
    httpx_mock.add_response(
        method="GET",
        url="https://issuer.example/userinfo",
        json={"sub": "subject", "email": "alice@example.com"},
    )

    userinfo = await oidc_service.OIDCUtils.get_userinfo("access-token")

    assert userinfo == {"sub": "subject", "email": "alice@example.com"}
    request = httpx_mock.get_request()
    assert request is not None
...
async def test_callback_request_rejects_ambiguous_state_before_core_flow():
    response = await oidc_service.oidc_callback_request_handler(
        {"state": ["first", "second"], "code": ["code"]},
        None,
        db=None,
    )

    assert response.status_code == 302
    assert "登录回调参数无效" in unquote(response.headers["location"])
...
async def test_oidc_provider_type_defaults_and_normalizes(monkeypatch):
    monkeypatch.setenv("OIDC_ENABLED", "true")
    monkeypatch.setenv("OIDC_PROVIDER_TYPE", " Standard ")

    config = oidc_service.OIDCConfig.from_env()

    assert config.provider_type == "standard"
