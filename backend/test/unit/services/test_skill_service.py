...
@pytest.mark.asyncio
async def test_skill_dependency_scope_covers_read_and_manage_audiences():
    parent = Skill(
        slug="parent",
        name="parent",
        description="parent",
    ...
    assert await svc.can_skill_depend_on(object(), parent, dependency) is True


@pytest.mark.asyncio
async def test_owner_only_skill_can_depend_on_visible_skill():
    parent = Skill(
...
async def test_personal_skill_cache_miss_scans_once_under_concurrency(
    ...
    assert 1 <= redis.lock_acquisitions[svc._personal_skill_scan_lock_key("user-1")] <= 2
