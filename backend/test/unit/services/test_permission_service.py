...
def test_default_roles_keep_existing_management_boundary():
    ...
    assert "knowledge.types.manage" in DEFAULT_ROLE_PERMISSIONS["admin"]
    assert "knowledge.types.manage" in DEFAULT_ROLE_PERMISSIONS["superadmin"]
    assert "knowledge.types.manage" not in DEFAULT_ROLE_PERMISSIONS["user"]
    assert "users.quota.manage" in DEFAULT_ROLE_PERMISSIONS["admin"]
    assert "users.quota.manage" in DEFAULT_ROLE_PERMISSIONS["superadmin"]
    assert "users.quota.manage" not in DEFAULT_ROLE_PERMISSIONS["user"]
    assert "users.quota.reset.global" in DEFAULT_ROLE_PERMISSIONS["superadmin"]
    assert "users.quota.reset.global" not in DEFAULT_ROLE_PERMISSIONS["admin"]
    assert "users.quota.reset.global" not in DEFAULT_ROLE_PERMISSIONS["user"]
