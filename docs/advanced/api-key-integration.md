...
API Key 的调用行为受两项新权限控制（在权限管理页可见、可分配）：

| 权限 | 作用 |
|------|------|
| `apikey.manage` | 可见并访问「API Key 管理」页；可对自己的 Key 执行 CRUD / 重新生成 |
| `apikey.invoke` | 关联用户的 API Key 在鉴权后可调用 Agent 对话接口 |

- **角色默认**：`admin` / `superadmin` 同时拥有这两项；`user` 默认不拥有。
- **使用 API Key 调用**（即 `Authorization: Bearer yxkey_...`）时，后端会强制要求关联用户拥有 `apikey.invoke`；否则返回 403，错误信息包含 `apikey.invoke`。
- **存量迁移**：升级到该版本后，admin/superadmin 的存量 Key 仍可继续使用；普通用户的存量 Key 调用会立即失败，需要管理员在权限管理页为该用户的角色勾选 `apikey.invoke` 后才会重新生效。
- **JWT 登录**请求不受此权限影响，仍然按用户自身的角色权限矩阵校验。
