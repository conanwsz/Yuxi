...
| `dept_code` | 部门编码 | `BM` 后每两位一级，兼容 `BM2203` 和 `JXI-BM4605`，唯一确定部门身份和父子关系。 |
| `dept_name` | 部门 OIDC 名称 | 只补充或更新当前编码节点的 OIDC 名称。 |

例如 `BM2203` 会解析为 `BM22 / BM2203`，`JXI-BM460503` 会解析为 `JXI-BM46 / JXI-BM4605 / JXI-BM460503`。缺少名称的父级先以编码展示，父级人员后续登录时补齐 OIDC 名称。管理员可设置独立的本地名称作为展示别名，但不能修改编码或移动 OIDC 节点。
