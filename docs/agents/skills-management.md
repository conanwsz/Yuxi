...
2. 未安装的推荐 Skill 会以套件卡片样式展示，标题下方显示套件名称、提供方与子 skill tag
3. 点击套件卡片后，系统会按套件配置的 `source`（如 `https://github.com/anthropics/skills`）拉取内容，并弹出「选择要安装的 skill」草稿
4. 选择个人工作区或共享 Skill 后完成安装

已安装的推荐 Skill 不会从「推荐」分组中移除——套件卡片始终显示，并在底部状态文案提示「已安装 N/M」。

**方式 1.5：发布到「推荐」用户共建池**

1. 任意入口（远程仓库 / 上传 zip / 推荐套件）的安装 modal 第 3 步「位置」多了一个「推荐工作区」选项
2. 选「推荐工作区」后，skill 落到 `skills` 表中（`is_recommended_workspace=True`），共享范围自动全局可读——**但不会自动装到任何工作区**
3. 该 skill 出现在「技能」tab 顶部「推荐」分组（与 admin 套件并列），卡片显示「由 {上传人} 上传」、状态「N 个可安装」+「查看并安装」——**和 admin 套件卡片表现完全一致**
4. 任何用户（包括 publisher 自己）都可以从「推荐」列表点击进入预览 modal，然后「安装到个人工作区」一键克隆到自己的 workspace
5. 装机人（与 admin）对该 skill 保留 manage 权限——可启停/删除；其他用户只读

「推荐」分组现在合并展示两类内容：admin 维护的精选套件 + 任意用户发布的 skill。publisher 自己要使用也需要去「推荐」列表里点装——这是为了保持所有 skill 的「未装 → 自选装」交互一致。

**管理员维护推荐套件**

拥有 `skills.recommend` 权限的管理员（admin / superadmin 默认获得）在「技能」tab 顶部会出现「管理推荐」按钮，可在此：

- 查看所有套件（含已停用），并对每个套件执行「编辑 / 启停 / 删除」
- 新建套件：填写 name / provider / source 后，可上传多 skill 的 zip 包（zip 顶层不放 SKILL.md，按子目录组织，每个子目录 1 个 SKILL.md）自动填充成员；也可手动添加成员
- 删除套件不会影响已装用户的 skill（已装 skill 存在 `skills` 表，与推荐表解耦）
...
1. **提示词注入**：系统在每次模型请求时动态注入可用 Skills 的描述与对应 `SKILL.md` 路径；目录说明只列出这些 Skill 实际使用的共享或个人路径，不把工作区标为更高优先级
