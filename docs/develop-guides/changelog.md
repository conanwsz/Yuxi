...
同一版本的多次功能更新时，应以功能为单位进行更新，比如之前添加了 A 功能的更新，在后续的更新中修复了因 A 功能引入的 bug，那么这个修复说明应该和 A 功能描述放在一起（重新修改表达，而仅仅是补充），而不是新增一条修复记录，功能更新同理。必须遵守：每一个修改不应超过 200 字，注意高度凝练。

## v0.7.2 (current)

- **浏览器工具 MCP（mcp-playwright + browser-viewer）**：支持智能体操作浏览器。新增 `mcp/playwright` 与 FastAPI viewer 容器，按 user_id 隔离 context；浏览器视图融合至会话“状态”面板，以 16:9 实时展示截图，页面快照仅超管可见且默认折叠；会话首次调用自动展开并尊重用户手动关闭意愿；仅工具实际调用刷新活跃时间，无操作 30 分钟后准时自动回收并释放浏览器资源。详见 [浏览器工具 MCP](../agents/browser-mcp.md)。
...
- **修复模型 Token 系数不生效**：`ModelInfo` 缓存未携带 `token_coefficient` 字段，`_resolve_token_coefficient` 始终回退到默认值 1.0，模型请求体里设的倍率在「个人 Token 用量」统计里完全失效。`ModelCache` dataclass 加 `token_coefficient` 字段，`rebuild()` 经 `_normalize_token_coefficient` 校验后写入，`to_dict`/`from_dict` 同步序列化（缺字段时回退 1.0 以兼容旧 Redis 条目）；写入侧 `_normalize_model_item` 同步校验倍率范围（0.01-100，精度 0.0001），改完即时生效、免重启。
...
- 修复编辑用户保存时报 422 错误的问题：在 `UserUpdate` 契约中补齐 `role` 字段，服务端增加修改用户角色的超管权限校验与唯一管理员降级保护；超管编辑自本人时不触发降级限制；前端优化角色选择器防误操作，并在保存自身时同步刷新前端当前登录信息；统一处理响应错误避免提示 `[object Object]`。

- 修复普通用户打开设置页时隐藏的 OCR、用户与部门管理组件仍提前请求受限接口并提示无权限的问题；桌面导航、移动导航、页签切换与内容挂载现统一按后端角色及权限判断。

- 用户管理新增单人、勾选多人和全局周额度重置；批量操作遵守额度权限与组织管理范围，全局重置使用独立权限且默认仅超管拥有。部门筛选升级为可搜索组织树，支持“包含下级 / 仅直属”，并同时匹配主部门和兼职部门；列表视图与卡片视图均支持跨分页选择。重置不改变额度上限并保留消费明细与操作日志。
- 用户管理列表去掉部门列兼职行，部门路径改为 12px 以压低行高；分页选项调整为 20/50/100/200/500。
...
- OIDC 登录新增静态 Provider 适配器层，通过 `OIDC_PROVIDER_TYPE` 选择单个认证中心；`cnnp` 适配器按公司、部门编码补齐组织树和占位父级，兼容 `BM2203` 与 `JXI-BM4605` 两类编码并在每次登录同步主部门。本地名称优先于 OIDC 名称展示，核心仍强制执行 state、PKCE、ID Token 验签及 `issuer + sub` 绑定。
...
- OIDC 用户设置新增系统只读 `entity_code` / `dept_code`：来自当前所属部门且与 `uid` 同等级别不可修改、未配置时不输出空字段，沙箱启动时同步注入；既有已绑定 OIDC 身份的用户下次访问即生效。
...
- Skill 推荐区升级为套件卡片，首批提供 Anthropic 文档处理套件；统一选择、短时加载、生效范围和结果四步弹窗，远程仓库与全局搜索保持在同一弹窗；失败重试会回传上游原始 Skill 名称，避免本地防重名 slug 导致 CLI 解析失败。
...
- 放开角色名称编辑：权限管理页的系统角色（含超级管理员）名称输入框不再 disabled；后端同步移除名称字段的 `is_system` 限制并将超级管理员的整体 403 收窄为「禁止修改权限与数据资源」，便于按企业内部习惯调整显示名称，鉴权、路由与用户-角色绑定仍以 `key` 为准不受影响。
...
- 新增 Agent backend 工具审批与完全信任模式：系统默认使用完全信任，Agent 配置可显式切换模式，聊天 run、Agent Call 与评估请求可用 `tool_approval_mode` 做单次覆盖，实际模式固化到 `AgentRun` 并由 resume/子智能体继承；请求审批仅拦截 `write_file`、`edit_file`、`execute`，只读 filesystem 工具直接执行，完全信任保持自动执行。前端输入框左下角新增线程级“请求审批 / 完全信任”选择器，支持刷新恢复显式选择；智能体配置页将带选项的 `string` 字段正确渲染为选择控件，模式解析按“线程显式选择、智能体显式配置、本地最近选择、系统默认值”依次回退，避免本地缓存覆盖智能体配置。同轮多个工具调用按顺序一次展示一项，参数默认只显示单行摘要，点击后带轻量动画在卡片内展开完整内容，展开区最大高度为 300px。审批卡进一步调整为直接覆盖输入区，按工具类型优先展示命令或目标路径，操作固定为“拒绝 / 允许”，审批期间底层输入控件不可交互。请求审批模式下子智能体隐藏敏感 backend 工具，避免绕过主线程逐项审批。补充模式解析、middleware、run 继承、事件压缩、子智能体过滤、前端状态恢复测试，并在真实页面验证刷新恢复、逐项批准或拒绝、参数点击展开与完全信任自动执行。模式解析合并为单次 context 共享（resolve\_agent\_run\_config）避免 run 创建/intake 重复加载上下文；resume 与子智能体继承统一读取固化快照，前端审批弹窗参数序列化改为 computed，并清理 `reviewConfigs` 等未消费状态。resume 解析 tool\_approval\_mode 时对旧版本固化、缺少该字段的 interrupted 运行保持请求审批语义，避免历史中断会话升级后扩大权限。
