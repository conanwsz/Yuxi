...
- 网页搜索支持多 backend fallback + 多 Tavily key 轮询/熔断：`YUXI_SEARCH_BACKEND` 升级为逗号分隔链（如 `tavily,duckduckgo` 或 `duckduckgo,tavily`），前一个 backend 失败时自动 fallback 到下一个；新增 `TAVILY_API_KEYS=key1,key2,key3` 启用多 key 池（兼容旧的 `TAVILY_API_KEY=key1`），多 key 按 round-robin 轮询使用，单个 key 命中 401/403/429 等"key 死了"信号时自动熔断 `YUXI_SEARCH_KEY_COOLDOWN` 秒（默认 60），冷却期内跳过该 key、冷却后自动恢复；5xx/timeout/connection error 视为 backend 级失败，整个 Tavily 跳过，落到链上下一个 backend。失败归一通过 `_classify_exception` 解析 `langchain_tavily` 抛出的 `ValueError("Error {status_code}: ...")` 消息和 `requests.HTTPError.response.status_code`；Tavily 工具层 `.invoke()` 会把异常吞进 `error` 字段，新加 `_classify_tavily_response` 检测并分类该字段，避免漏掉熔断。所有 backend 都失败时返回 `results=[]` + `error` 字段（不抛异常），让 Agent 看到失败原因；chain 层记录 warning 日志便于诊断。`test_search.py` 增至 38 个用例覆盖 key 池 round-robin/熔断/恢复/去重、异常分类、链式 fallback、空 results 不触发 fallback、假 key 端到端轮询等场景。
...
- 会话页 UX 修复：发送消息后右侧立刻从"开始界面"切到"会话视图"（不再短暂停留在 `randomGreeting`），欢迎界面渲染条件从 `!conversations.length` 收紧为 `!currentChatId`；Token 超额（HTTP 429 `token_quota_exceeded`）、权限不足（HTTP 403，含 API Key 鉴权失败）与其他 4xx/5xx 错误改用 `ChatAlertBanner` 内联展示在会话流末尾。Token 超额在 run 创建前会把用户消息、失败状态和结构化额度详情写入服务端会话历史，刷新或跨设备进入该会话时可恢复消息、唯一红框和重试上下文；浏览器会话存储仅作为网络异常时的兜底。点击重试时，之前失败的用户消息会按原始顺序作为本次请求的上下文与最后一条消息一起送入 Agent；服务端只会把明确列入重试上下文的失败消息转为完成，用户改发新消息成功时则将旧失败记录标记为已替代并从历史、Agent 上下文和告警中退出；成功创建 run 会同时清除同会话的旧浏览器告警与本地失败消息，加载历史时也会移除已被稍后同内容成功请求替代的旧本地副本，避免失败问题长期重复挂在会话尾部。重试操作为单飞执行，流结束前重复点击会被忽略；run 已创建并正在生成时隐藏历史错误快照，若重试再次失败则在流结束后显示最新错误；同类告警始终合并为一条并更新最新重试载荷。
- 模型调用错误脱敏：主智能体和子智能体的模型重试耗尽后改为进入失败链路，不再把异常类型、模型服务 URL、IP、端口和上游响应体保存成普通 AI 回答；直接聊天、resume、后台 run 事件与 run 终态统一只返回安全中文文案，完整原始异常仅保留在带 run/thread/request 标识的服务端日志中。开发和生产环境执行相同的用户侧脱敏规则，前端会将无 HTTP 状态的结构化运行错误作为会话内错误展示。
...
- 精简登录页品牌与辅助信息：顶部仅展示组织品牌名称，移除用户协议确认、帮助链接及版权页脚；账号密码登录、OIDC 登录和首次初始化不再依赖协议勾选状态。
...
- 收紧 DeepAgents `read_file` 的非文本读取边界：图片继续以多模态块返回，并由 OpenAI 兼容模型请求镜像为 user-role 图片；XLSX/XLS 等表格与 Office、PDF、ZIP 等通用二进制不再以 base64 `file/document` 块进入模型，而是返回 sandbox `execute`/专用解析工具指引。附件提示按类型分流，临时表格附件支持生成 Markdown 预览并保留原始路径；新增模型调用前净化中间件，使已保存无效文件工具块的历史线程也能继续运行。
- 修复对话模型选择回显：AgentRun 输入消息持久化本次实际 `model_spec`，历史接口同时返回线程最近一次 run 的模型；重新进入旧线程时，前端优先使用该值恢复选择，不再错误回退到智能体默认模型。
- 首页移除 GitHub 入口、Stars 徽章及统计卡片，取消对应的 GitHub 实时请求；全局主色由墨绿色调整为与机器人 Logo 一致的深蓝色，首页主按钮、品牌标题和登录强调色同步更新。
- 浏览器页面标题更新为“超级楚楚 - 楚能AI平台”。
- 修复模型选择器回显：前台按钮现在复用模型管理中的显示名称，不再将内部模型标识直接展示给用户。
