...
对话模型可单独配置 Token 计费倍率（仅 chat 模型，输入范围 0.01-100），用于按模型灵活调整「个人 Token 用量」统计与周配额扣减：

- 模型实际消耗 100 tokens、倍率 0.5 → 用户用量记 50（节省配额）
- 模型实际消耗 100 tokens、倍率 2.0 → 用户用量记 200（更"贵"）

倍率**即时生效**，保存后下一次聊天回调即可看到新值写入，无需重启服务。本周已累计的用量不会追溯重算。

常见用途：

- 把官方自有推理集群（如内网 Qwen3.5）按低于 1.0 的倍率登记，区分计费成本
- 对接价格更高的供应商或大模型时按 > 1.0 收口配额
- 给试用模型设较低倍率，鼓励组内使用

倍率只影响本系统的「个人 Token 用量」统计与周配额扣减，不改变上游供应商的实际计费。
